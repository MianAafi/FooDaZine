...
import '../Components/Css/FoodDescription.css';
...
function FoodDescription() {
  ...
  function DeleteGallery(public_id) {
    const formData = new FormData();
    formData.append('public_id', public_id);
    formData.append('items_id', items._id);
    Swal.fire({
      title: 'Are you sure?',
      ...
      if (result.isConfirmed) {
        instance
          .post(`/items/api/image/delete`, formData)
          .then((response) => {
            setItems(response.data);
          })
          .catch((error) => {
                ...
                >
                  <div className="Galleryimages-size">
                    <i
                      ...
                      class="fa fa-times"
                      aria-hidden="true"
                      onClick={() => DeleteGallery(image[0].public_id)}
                    />
                    <img
                      src={image[0].url}
